 ...
 */
let config = {
    darksky_api: "4450f6ce6eb357607e832e1ead9599cd", //从https://darksky.net/dev/ 上申请key填入即可
    aqicn_api: "b8ae7bd3cd505aaef576a26fb7cd873ab1683c0b", //从http://aqicn.org/data-platform/token/#/ 上申请key填入即可
    huweather_apiKey: "0d56d4d691a946d5941516744d1fa158", //和风天气APIkey,可自行前往 https://dev.heweather.com/ 进行获取(注意key类型选WebApi)
    lat_lon: "22.958824710745873,113.08931412736229", //请填写经纬度,直接从谷歌地图中获取即可
    lon_lat: "113.08931412736229,22.958824710745873", //请填写经纬度,直接从谷歌地图中获取即可
    lang: "ja", //语言,请不要修改
    show: {
...
function heweatherLifestyle() {
    ...
    if (needRequest) {
        var hurl = {
            url: `https://free-api.heweather.net/s6/weather/lifestyle?location=${config.lon_lat}&key=${config.huweather_apiKey}&lang=${config.lang}`,
        };
...
`;
    if (config.show.aqi) {
        notifyInfo.detail += `${notifyInfo.detail==""?"":lineBreak}${config.show.icon?'😷':''}空气质量 ${weatherInfo.aqiInfo.aqi}(${weatherInfo.aqiInfo.aqiDesc}) ${config.show.icon?'💨':''}风速${weatherInfo.dailyInfo.windSpeed}km/h ${weatherInfo.dailyInfo.ozone}`;
    }
    if (config.show.uv) {
        notifyInfo.detail += `${notifyInfo.detail==""?"":lineBreak}${config.show.icon?'🌚':''}紫外线指数${weatherInfo.dailyInfo.uvIndex}(${getUVDesc(weatherInfo.dailyInfo.uvIndex)}) ${config.show.icon?'💦':''}湿度${weatherInfo.dailyInfo.humidity}%`;
    }
    if (config.show.apparent) {
        notifyInfo.detail += `${notifyInfo.detail==""?"":lineBreak}${config.show.icon?'🌡':''}体感温度${Math.round(weatherInfo.dailyInfo.apparentTemperatureLow)} ~ ${Math.round(weatherInfo.dailyInfo.apparentTemperatureHigh)}℃ ${config.show.icon?'🏋🏻‍♀️':''}压力${weatherInfo.dailyInfo.pressure}pa`;
    }
